...
def apparition_donnée():
    ...
    point2.configure(text=score_a)
    distance2.configure(text=distance_a)
    temps2.configure(text=f"{int(temps_a//60)}min ; {int(temps_a%60)}s")
...
def play():
    ...
    score_b = resultat_mouvement[0]
    temps_b = resultat_mouvement[1]
    distance_b = resultat_mouvement[2]
